    ...
    while (framework->isTagConnected()) {
        ...
        switch (waitForEvent()) {
            ...
            case CodeEntered:
                ...
                if (code == "" || code == KEYBOARD_CLEAN) break; // break switch -> repeat

                const PriceListEntry *entry = priceList->getByCode(code);
                if (entry != nullptr) { // found!
                    Debug.printf("Pricelist item found: name=%s price=%d delta=%d\n", entry->name.c_str(), entry->price, entry->delta);
...
bool Portal::stage3(const PriceListEntry *entry) {
    ...
    Debug.printf("Charging item: name=%s price=%d delta=%d\n", entry->name.c_str(), entry->price, entry->delta);

    const Transaction t = Transaction{framework->clocks.getCurrentTime(), static_cast<u8>(currentPlayerData.user_id), entry->delta};
    ...
    currentPlayerData.strength += entry->delta;
